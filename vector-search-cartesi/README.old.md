...
```shell
DAPP_NAME=vector-search docker compose -f ./deploy-testnet.yml up
```

This will create a file at `./deployments/<network>/vector-search.json` with the deployed contract's address.
Once the command finishes, it is advisable to stop the docker compose and remove the volumes created when executing it.

```shell
DAPP_NAME=vector-search docker compose -f ./deploy-testnet.yml down -v
```
...
```shell
yarn start input send --payload "Hello there" --addressFile path/to/vector-search/deployments/<network>/vector-search.json
```
...
```shell
ls *.py | ROLLUP_HTTP_SERVER_URL="http://127.0.0.1:5004" entr -r python3 vector-search.py
```
